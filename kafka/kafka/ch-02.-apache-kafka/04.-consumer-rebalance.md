...
* 동일한 [group.id](http://group.id) 로 구성된 모든 Consumer들은 하나의 Consumer Group을 형성
...
* hash([group.id](http://group.id)) % offsets.topic.num.partitions 수식을 사용하여 group.id가 저장될 \_\_consumer\_offsets 의 Partition을 결정
